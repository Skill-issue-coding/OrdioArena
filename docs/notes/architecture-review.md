> **Status:** Findings, 2026-07-02 review · **Tracking:** [#30](https://github.com/Skill-issue-coding/OrdioArena/issues/30) · **Updated:** 2026-08-20
>
> Findings only. The original "Recommended order" section was removed, sequencing now lives in the
> issues. Item §1 (reconnect) is specified in [`../design/0001-reconnect.md`](../design/0001-reconnect.md);
> item §2 (data races on `Client` fields) is absorbed by the seat model in
> [#18](https://github.com/Skill-issue-coding/OrdioArena/issues/18) rather than tracked separately.
...
- `project-context.md` (deleted; its content now lives in the root `CLAUDE.md`) described the client as Next.js with `client/components/home/HomeView.tsx` paths, actual is Vite + TanStack Router. `frontend/CLAUDE.md` still says "No routes defined yet" but `routeTree.gen.ts` exists; update it.
