...
function ModeExample({ modeId }: { modeId: GameMode }) {
  if (modeId === "impostor") {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
        <div className="p-4 rounded-xl border-2 bg-card shadow-sm" style={{ borderColor: "color-mix(in srgb, var(--color-game-green) 40%, var(--color-border))" }}>
          <div className="text-[10px] font-display font-extrabold uppercase tracking-widest text-game-green mb-1">3 av 4 spelare</div>
        ...
        <div className="p-4 rounded-xl border-2 bg-card shadow-sm" style={{ borderColor: "color-mix(in srgb, var(--color-game-red) 40%, var(--color-border))" }}>
          <div className="text-[10px] font-display font-extrabold uppercase tracking-widest text-game-red mb-1">Impostorn 🕵️</div>
  ...
  if (modeId === "contexto_battle") {
    ...
    return (
      <div className="flex flex-col gap-2 mt-6">
        <div className="text-[10px] font-display font-extrabold uppercase tracking-widest text-muted-foreground mb-1">Exempel: målord = "hav"</div>
        {guesses.map((g) => (
          <div key={g.w} className={cn("flex justify-between items-center px-4 py-2.5 rounded-xl text-white font-display font-bold shadow-sm", g.bg)}>
  ...
  if (modeId === "synonym_duel") {
    ...
    return (
      <div className="mt-6">
        <div className="text-[10px] font-display font-extrabold uppercase tracking-widest text-muted-foreground mb-2">Exempel: målord = "fordon"</div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {players.map((p) => (
            <div key={p.n} className={cn("p-3 rounded-xl border-2 bg-card shadow-sm transition-opacity", p.kept ? "border-border" : "border-game-red opacity-60")}>
              ...
              <div className={cn("text-[10px] font-mono font-bold tracking-tight", p.kept ? "text-game-green" : "text-game-red")}>
  ...
  if (modeId === "anti_match") {
    ...
    return (
      <div className="mt-6">
        <div className="text-[10px] font-display font-extrabold uppercase tracking-widest text-muted-foreground mb-2">Exempel: målord = "äpple"</div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {players.map((p) => (
            <div key={p.n} className={cn("p-3 rounded-xl border-2 shadow-sm", p.dup ? "border-game-red bg-game-red/5" : p.pts > 80 ? "border-game-green bg-card" : "border-border bg-card")}>
              <div className="text-[10px] font-display font-bold text-muted-foreground mb-1">{p.n}</div>
              <div className="text-lg font-display font-extrabold leading-none mb-2 text-foreground">{p.w}</div>
              <div className={cn("text-[10px] font-mono font-bold tracking-tight", p.dup ? "text-game-red" : "text-game-green")}>{p.dup ? "DUBBLETT · 0p" : `+${p.pts}p`}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <DialogContent className="sm:max-w-3xl md:max-w-7xl w-[95vw] max-h-[85vh] overflow-y-auto bg-background p-6 md:p-8 border-2 border-border rounded-2xl shadow-xl outline-none">
        <DialogTitle asChild>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-2">
            <h1 className="text-3xl md:text-4xl font-extrabold font-display text-game-purple tracking-tight">Snabb guide</h1>
            <span className="font-display font-bold text-xs text-muted-foreground uppercase tracking-widest">Hur fungerar OrdioArena?</span>
          </div>
        </DialogTitle>
          ...
          <div className="flex flex-col sm:flex-row gap-5 items-start">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm font-body font-semibold text-muted-foreground leading-relaxed">
                OrdioArena bygger på <strong className="text-foreground">fastText-vektorer för svenska ord</strong>. Varje ord du skriver placeras i ett 100-dimensionellt rum. Spelet räknar avstånd mellan ditt ord och målordet — och det avgör vinnaren.
              </p>
              <div className="flex gap-2 mt-4 flex-wrap">
                {["~200k ord", "spaCy POS-filter", "Kelly + Korp + Maktbarometern", "WebSocket realtid", "Go backend"].map((tag) => (
                  <span key={tag} className="font-display font-bold text-[10px] uppercase tracking-wider px-2.5 py-1.5 rounded-md bg-card border-2 border-border text-muted-foreground shadow-sm">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? `bg-game-${m.color} border-game-${m.color} text-white shadow-md scale-105` : "bg-muted/40 border-border text-muted-foreground hover:border-muted-foreground/40 hover:bg-muted/60",
                )}>
        ...
        <div
          ...
          <div className="flex items-center gap-4 mb-2">
            <div className={cn("w-16 h-16 rounded-lg flex items-center justify-center text-3xl shadow-sm", `bg-game-${mode.color}`)}>{mode.icon}</div>
            <div>
              <h2 className={cn("text-2xl font-display font-extrabold", mode.textClass)}>{mode.title}</h2>
              <div className="text-xs font-display font-bold text-muted-foreground mt-1 uppercase tracking-wider">{mode.players}</div>
            </div>
          </div>
          ...
          <div>
            <div className="text-[10px] font-display font-extrabold text-muted-foreground uppercase tracking-widest mb-3">Faser per omgång</div>
            <div className="flex items-center gap-2 flex-wrap">
              {info.phases.map((phase, i) => (
                <div key={i} className="flex items-center gap-2">
                  <div className="px-3 py-1.5 rounded-xl border-2 bg-background font-display font-bold text-xs shadow-sm flex items-center gap-2" style={{ borderColor: `color-mix(in srgb, var(--game-${mode.color}) 40%, var(--border))` }}>
                    <span className={mode.textClass}>{i + 1}</span>
                    <span className="text-foreground">{phase}</span>
                  </div>
                  {i < info.phases.length - 1 && <ArrowRight className="w-4 h-4 text-muted-foreground/50 shrink-0" strokeWidth={3} />}
                </div>
              ))}
            </div>
          </div>
